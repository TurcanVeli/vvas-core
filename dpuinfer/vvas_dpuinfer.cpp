...
parse_prototxt_file (VvasDpuInferPrivate * kpriv, const std::string & prototxt_file)
{
  ...
  CHECK (ok) << "Config file parse edilemedi. prototxt_file=" << prototxt_file;
...
modelexists (VvasDpuInferPrivate * kpriv)
{
  ...
  if (kpriv->modelclass != VVAS_XCLASS_RAWTENSOR && !fileexists (prototxt_name)) {
    LOG_MESSAGE (LOG_LEVEL_ERROR, kpriv->log_level, "%s bulunamadı :(",
        prototxt_name.c_str ());
    elf_name = "";
    return elf_name;
  }
...
readlabel (VvasDpuInferPrivate * kpriv, char *json_file)
{
  ...
  if (!value || !json_is_integer (value)) {
    LOG_MESSAGE (LOG_LEVEL_ERROR, kpriv->log_level,
        "dosya( %s )  =>  dosyanın içinde num-labels bulunamadı", json_file);
    goto error;
  } else {
...
#endif
#ifdef ENABLE_YOLOV6
    case VVAS_XCLASS_YOLOV6:
    {
      model = new vvas_yolov6 (kpriv, kpriv->elfname, kpriv->need_preprocess);
      break;
    }
#endif
